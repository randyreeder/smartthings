...
This guide explains how to connect your Garmin ConnectIQ app to the SmartThings PHP API server. It covers both Personal Access Token and OAuth (API Key) authentication, seamless setup flows, and best practices for secure, long-term operation.

For general setup, authentication, and deployment, see [SETUP_AND_SECURITY.md](SETUP_AND_SECURITY.md).

---

## Authentication Methods for Garmin

### Method 1: Personal Access Token (Simple, requires creating a token)

**What to enter in your Garmin watch app settings:**
- **Server URL**: `https://yourserver.com/weather/smartthings/json.php`
- **Token**: Your personal token (36-character UUID)
...
### Method 2: OAuth Authorization (Recommended, no manual token creation)

**What to enter in your Garmin watch app settings:**
- **Server URL**: `https://yourserver.com/weather/smartthings/json.php`
- **API Key**: 64-character key generated by server during OAuth setup

#### Seamless Setup Process (No Manual Copy/Paste)
1. Watch app starts OAuth flow by opening browser to setup URL
2. Watch app gets Session ID from setup page and starts polling
3. User completes OAuth in browser
4. Watch app automatically receives API key via polling
5. Done! Watch app saves and uses the API key

#### Manual Setup Process (Fallback)
1. Visit setup URL in browser
2. Note your Session ID
3. Complete OAuth and copy API key from success page
4. Manually enter API key into Garmin watch app

---

## System Architecture Overview

The SmartThings API server stores credentials and tokens securely outside the web root. See [SETUP_AND_SECURITY.md](SETUP_AND_SECURITY.md) for details.

---

## Developer Reference: API Usage, Polling, and Token Refresh
